...
object bussinessLogics {
  ...
  def sellingChannelHandler(df:DataFrame,tableName:String): DataFrame ={
  ...
  def retailPriceHandler(valid_channel_df:DataFrame,tableName:String): DataFrame ={
  ...
  def productIdHandler(spark:SparkSession,removed_null_price_df:DataFrame,tableName:String): DataFrame ={
...
*/
    ...
    producerConsumer.producerCSV.prodcue(spark,"D:/TargetCoorporationPhaseSecond/Data/targetReadyDataSet.csv","kafka","localhost:9092","datastreamTest7")

    val df = producerConsumer.newConsumerCSV.consume(spark,"kafka","localhost:9092","datastreamTest7")

    df.show(false)

    /** HANDLING  SELLING CHANNELS*/
    val valid_channel_df = sellingChannelHandler(df,"error_selling_channel")

    /**Handling retailPrice*/
    val removed_null_price_df = retailPriceHandler(valid_channel_df,"error_retail_price")

    /**Handling ProductID*/
    val final_df = productIdHandler(spark, removed_null_price_df,"error_product_id")

...
import template.spark.dbConnection.errorTableWriter

object businessLogic {
  def main(args: Array[String]): Unit = {

    /** *******************************Reading the JSON data and converting to dataframe format**************** */
...
/***************************************************  Changing the dataype   *********************************************************************************/
    ...
    // 1st way
    // val sellingDF = updatedDF.filter(col("selling_channel")==="Cross Over"||col("selling_channel")==="Store Only"|| col("selling_channel")==="Online Only")

    //val errorDf= updatedDF.filter(col("selling_channel")!="Cross Over" && col("selling_channel")!="Store Only" && col("selling_channel")!="Online Only")
    ...
    //   the result is  means all record
    println(nullCheckDf.count())
    println(validProduct_idDF.count())
    //  the count of both table r same means all records given to us have numeric 8 digit for product_id

    /** ***************************************Remove duplicate if any based on product_id and location_id **************************************************** */
    val deduplicateDF = validProduct_idDF.dropDuplicates("product_id", "location_id")
    println(deduplicateDF.count())
    //    same As above count so no duplicate was present in validproductDF
